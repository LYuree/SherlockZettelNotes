...
class Field(object):
    ...
    def print_cell(self, elem):
        #destroyed
        if(elem == 'X'):
            color.write(elem, "COMMENT")
        #empty (also miss) - for the computer's board
        elif(elem == '0'):
            color.write(elem, "STRING")
        #shot
        elif(elem == '!'):
            color.write(elem, "KEYWORD")
...
class Player(object):
    ...
    def set_hit_counter(hit_count):
        self.hit_count = hits

    def announce_combo(self, hit_count):
        #color.write(str(hit_count), "COMMENT")
        if(shot_result != 'retry'):
            if(hit_count == 2):
                color.write('Высокая меткость! ', "STRING")
            elif(hit_count == 3):
                color.write('Превосходно! ', "KEYWORD")
            elif(hit_count >= 4):
                color.write('РЕЗНЯ!!! ', "COMMENT")
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if game.status == 'in game':
            ...
            """
            #*************************
            #checking loss procession
            game.current_player.ships = []
            #*************************
            """
            #assigning messages depending on the result; if missed, passing the turn to the
            #next player
            if shot_result == 'miss':
