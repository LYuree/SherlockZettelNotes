...
class Player(object):
    ...
    def get_input(self, input_type, inputStr = '', i = 0):
        ...
        if(input_type == 'action'):
            if(self.is_ai):
                self.current_weapon = 'basic'
                action_result = self.get_input('shot')
                user_input = inputStr
            else:            
                user_input = input().upper().replace(" ", "")
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if game.status == 'in game':
            ...
            #ВЕРНУТЬ ОБРАБОТКУ РЕЗУЛЬТАТА ВЫСТРЕЛА "РЕТРАЙ",
            # А ТО У НАС УДАРЫ ПО ОТЫГРАННЫМ КЛЕТКАМ НЕ ЗАПРЕЩАЮТСЯ
            if game.current_player.shot_result == 'retry':
                game.current_player.message.append('Эта клетка уже сыграна, по ней нельзя совершить выстрел.'+
                                                    '\nПопробуйте её раз!')
                continue
